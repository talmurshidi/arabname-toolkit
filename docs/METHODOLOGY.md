...
**Divine names — `Allāh`/`al-Raḥmān`/`al-Raḥīm`:** these are dictionary entries (not character-level engine rules) using the traditional Qur'anic/Classical orthography — alif wasla (`ٱ`, U+0671) for the elidable hamzat al-waṣl, and (for `Allāh`/`al-Raḥmān`, which omit a written root alif) dagger alif (`ٰ`, U+0670) — rather than a plain-alif spelling. This is not exclusive to Qur'anic quotation; it is the standard spelling of these specific words in Arabic generally. Case-ending forms (`Allāhi`, `al-Raḥmāni`, etc.) and the hyphenated elided spelling Brill sources use when preceded by another word (`al-lāhi`, as in `Bismi al-lāhi al-raḥmāni al-raḥīmi`, the Basmala) are all curated entries; the Basmala phrase itself is a verbatim entry sourced from the Uthmani Mushaf text of Q1:1. Existing compounds containing these words (`ʿAbdallāh`, `ʿAbd al-Raḥmān`, `ʿAbd al-Raḥīm`, etc.) use the same spelling for consistency. `al-Raḥīm` has no omitted root alif, so outside the exact Basmala quote it keeps this project's own yā-sukūn house style on its medial `ي` like any other `-īm`/`-īd` word (see `arabicHouseStyle.ts` below).
...
4. **`ʿ` vs backtick vs ASCII apostrophe vs curly quotes.** The `fixBrillChar()` function normalises backtick (`` ` ``) to `ʿ`, ASCII apostrophe (`'`) to `ʾ`, and the typographic curly quotes — ‘ (U+2018) to `ʿ` / ’ (U+2019) to `ʾ` — the latter pair because real bibliographic sources (Brill/EI-style Latinised name lists) typeset ʿayn/hamza that way. The left curly quote is disambiguated from its other historical use as the elided-article marker ("Abū ‘l-Qāsim") before the ʿayn mapping is applied. The correct Unicode characters (U+02BF, U+02BE) are always used in output. This blanket left/right mapping is itself a heuristic, not a semantic parse: ordinary "smart quote" typesetting picks the left curly quote at the start of a word and the right one elsewhere, _regardless_ of whether hamza or ʿayn is actually intended — so a word-medial ʿayn typeset as a right curly quote (e.g. "Ismā’il" for "Ismāʿīl") is misread as hamza by this blanket rule. A small, explicit set of dictionary aliases (`Ismāʾil`, `Maʾshar`, `Jaʾfar`, `Yaʾqūb` — see `dictionary.ts` and `CHANGELOG.md`) corrects the specific words this has been observed on; it is not a general disambiguation and any other word with a genuine word-medial ʿayn typeset this way will still be misread unless it too gets an alias.
